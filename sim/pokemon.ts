...
export class Pokemon {
	...
	constructor(set: string | AnyObject, side: Side) {
		...
		for (const [index, moveid] of this.set.moves.entries()) {
			...
			// Get PP Ups for this specific move (default to 0)
			let ppUpsUsed = 0;
			if (this.set.ppUps && this.set.ppUps.length > index) {
				ppUpsUsed = this.set.ppUps[index];
			}
			
			// Clamp PP Ups to valid range (0-3)
			ppUpsUsed = Math.max(0, Math.min(3, ppUpsUsed));

			const ppBonus = Math.floor(move.pp * 0.2 * ppUpsUsed);
			let basepp = move.noPPBoosts ? move.pp : move.pp + ppBonus;
