...
export class Battle {
	...
	runAction(action: Action) {
		...
		case 'use': {
			...
			if (!pokemon || !action.item) return;

			this.add('-use', pokemon.side.id, action.item);

			const itemId = toID(action.item);

			const medicineEffects: Record<string, { 
				heal?: number; 
				curesStatus?: string[]; 
				restoresPP?: number | 'all' | 'max'; 
				targetMove?: 'one' | 'all';
				revive?: boolean;
				lowersFriendship?: boolean;
				reviveAll?: boolean;
			}> = {
				// HP HEALING ITEMS
				'potion': { heal: 20 },
				...
				'maxpotion': { heal: pokemon.maxhp },
				'fullrestore': { heal: pokemon.maxhp, curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				
				// ALTERNATIVE HP HEALING
				'freshwater': { heal: 50 },
				'sodapop': { heal: 60 },
				'lemonade': { heal: 80 },
				'moomoomilk': { heal: 100 },
				...
				// HP HEALING (LOWERS FRIENDSHIP)
				'energypowder': { heal: 50, lowersFriendship: true },
				'energyroot': { heal: 200, lowersFriendship: true },
				
				// STATUS CONDITION HEALING
				'antidote': { curesStatus: ['psn', 'tox'] },
				'burnheal': { curesStatus: ['brn'] },
				'iceheal': { curesStatus: ['frz'] },
				'awakening': { curesStatus: ['slp'] },
				'paralyzeheal': { curesStatus: ['par'] },
				'fullheal': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				
				// STATUS HEALING (LOWERS FRIENDSHIP)
				'healpowder': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'], lowersFriendship: true },
				
				// REGIONAL STATUS HEALING ITEMS
				'lavacookie': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'oldgateau': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'casteliacone': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'lumiosegalette': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'shaloursable': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'bigmalasada': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'pewtercrunchies': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				'ragecandybar': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				
				// PP RECOVERY ITEMS
				'ether': { restoresPP: 10, targetMove: 'one' },
				'maxether': { restoresPP: 'max', targetMove: 'one' },
				'elixir': { restoresPP: 10, targetMove: 'all' },
				'maxelixir': { restoresPP: 'max', targetMove: 'all' },
				
				// REVIVAL ITEMS
				'revive': { revive: true, heal: Math.floor(pokemon.maxhp / 2) },
				'maxrevive': { revive: true, heal: pokemon.maxhp },
				'revivalherb': { revive: true, heal: pokemon.maxhp, lowersFriendship: true },
				'sacredash': { revive: true, heal: pokemon.maxhp, reviveAll: true },
				
				// FLUTES (Gen III-IV, ORAS)
				'blueflute': { curesStatus: ['slp'] },
				'redflute': { curesStatus: ['infatuation'] },
				'yellowflute': { curesStatus: ['confusion'] },
				
				// BERRIES (Status Healing)
				'cheriberry': { curesStatus: ['par'] },
				'chestoberry': { curesStatus: ['slp'] },
				'pechaberry': { curesStatus: ['psn', 'tox'] },
				'rawstberry': { curesStatus: ['brn'] },
				'aspearberry': { curesStatus: ['frz'] },
				'persimberry': { curesStatus: ['confusion'] },
				'lumberry': { curesStatus: ['brn', 'par', 'psn', 'tox', 'frz', 'slp'] },
				
				// SPECIAL ITEMS
				'mentalherb': { curesStatus: ['infatuation', 'taunt', 'encore', 'torment', 'disable'] },
			};

			if (itemId in medicineEffects) {
				const effect = medicineEffects[itemId];
				let itemUsed = false;

				// Revival items
				if (effect.revive && pokemon.fainted) {
					if (effect.reviveAll) {
						for (const teamPokemon of pokemon.side.pokemon) {
							if (teamPokemon.fainted) {
								teamPokemon.fainted = false;
								teamPokemon.hp = effect.heal || Math.floor(teamPokemon.maxhp / 2);
								this.add('-heal', teamPokemon, teamPokemon.getHealth, `[from] item: ${action.item}`);
							}
						}
						itemUsed = true;
					} else {
						pokemon.fainted = false;
						pokemon.hp = effect.heal || Math.floor(pokemon.maxhp / 2);
						this.add('-heal', pokemon, pokemon.getHealth, `[from] item: ${action.item}`);
						itemUsed = true;
					}
				}
				// HP healing
				else if (effect.heal && !pokemon.fainted && pokemon.hp < pokemon.maxhp) {
					pokemon.heal(effect.heal);
					this.add('-heal', pokemon, pokemon.getHealth, `[from] item: ${action.item}`);
					itemUsed = true;
				}

				// Status curing
				if (effect.curesStatus && pokemon.status) {
					for (const status of effect.curesStatus) {
				...
				// PP restoration
				if (effect.restoresPP && !pokemon.fainted) {
					if (effect.targetMove === 'one') {
						for (const moveSlot of pokemon.moveSlots) {
							if (moveSlot.pp < moveSlot.maxpp) {
								const ppToRestore = effect.restoresPP === 'max' ? 
									moveSlot.maxpp - moveSlot.pp : 
									Math.min(effect.restoresPP as number, moveSlot.maxpp - moveSlot.pp);
								
								moveSlot.pp += ppToRestore;
								this.add('-message', `${pokemon.name}'s ${moveSlot.move} had its PP restored by ${ppToRestore}!`);
								itemUsed = true;
								break;
							}
						}
					} else if (effect.targetMove === 'all') {
						for (const moveSlot of pokemon.moveSlots) {
							if (moveSlot.pp < moveSlot.maxpp) {
								const ppToRestore = effect.restoresPP === 'max' ? 
									moveSlot.maxpp - moveSlot.pp : 
						...
						if (itemUsed) {
							this.add('-message', `${pokemon.name}'s moves had their PP restored!`);
						}
					}
				}

				if (effect.lowersFriendship && itemUsed) {
					pokemon.happiness = Math.max(0, pokemon.happiness - 15);
				}

				// Battle conditions
				if (effect.curesStatus && !pokemon.fainted) {
					for (const condition of effect.curesStatus) {
						if (condition === 'confusion' && pokemon.volatiles['confusion']) {
							pokemon.removeVolatile('confusion');
							this.add('-end', pokemon, 'confusion', `[from] item: ${action.item}`);
							itemUsed = true;
						}
						if (condition === 'infatuation' && pokemon.volatiles['attract']) {
							pokemon.removeVolatile('attract');
							this.add('-end', pokemon, 'Attract', `[from] item: ${action.item}`);
							itemUsed = true;
						}
						if (condition === 'taunt' && pokemon.volatiles['taunt']) {
							pokemon.removeVolatile('taunt');
							this.add('-end', pokemon, 'Taunt', `[from] item: ${action.item}`);
							itemUsed = true;
						}
						if (condition === 'encore' && pokemon.volatiles['encore']) {
							pokemon.removeVolatile('encore');
							this.add('-end', pokemon, 'Encore', `[from] item: ${action.item}`);
							itemUsed = true;
						}
						if (condition === 'torment' && pokemon.volatiles['torment']) {
							pokemon.removeVolatile('torment');
							this.add('-end', pokemon, 'Torment', `[from] item: ${action.item}`);
							itemUsed = true;
						}
						if (condition === 'disable' && pokemon.volatiles['disable']) {
							pokemon.removeVolatile('disable');
							this.add('-end', pokemon, 'Disable', `[from] item: ${action.item}`);
							itemUsed = true;
						}
					}
				}
				...
				if (!itemUsed) {
					this.add('-message', `${pokemon.name} used ${action.item}, but nothing happened.`);
				}
