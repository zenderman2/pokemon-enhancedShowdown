...
export interface ChosenAction {
	choice: 'move' | 'switch' | 'instaswitch' | 'revivalblessing' | 'team' | 'shift' | 'pass' | 'use';// action type
	item?: string,
...
export class Side {
		...
		for (const choiceString of choiceStrings) {
			...
			case 'use':
				if (data) {
					...
					if (!isNaN(parseInt(lastPart)) && parseInt(lastPart) > 0) {
						targetSlot = parseInt(lastPart);
					...
					if (!this.chooseUseItem(itemName, targetSlot)) return false;
				} else {
					return this.emitChoiceError(`No item specified for use`);
				}
				break;
			case 'auto':
			case 'default':
	...
	chooseUseItem(itemName: string, targetSlot?: number): boolean | Side {
		this.clearChoice();
		
		let pokemon;
		if (targetSlot !== undefined) {
			if (targetSlot >= 1 && targetSlot <= this.pokemon.length) {
				pokemon = this.pokemon[targetSlot - 1];
			} else {
				return this.emitChoiceError(`Invalid Pokémon slot: ${targetSlot}`);
			}
		} else {
		...
		if (!pokemon) {
			return this.emitChoiceError(`No Pokémon found at the specified slot`);
		}
	...
	choosePass(): boolean | Side {
		...
		case 'move':
			if (!pokemon.fainted && !pokemon.volatiles['commanding']) {
				return this.emitChoiceError(`Can't pass: Your ${pokemon.name} must make a move (or switch) But I'm passing anyway`);
			}
			break;
		default:
