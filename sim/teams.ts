...
export const Teams = new class Teams {
	pack(team: PokemonSet[] | null): string {
		...
		for (const set of team) {
			...
			if (set.ppUps && set.ppUps.length > 0 && set.ppUps.length <= 4) {
				buf += `|${set.ppUps.join(',')}`;
			} else {
				buf += '|';
			}
	...
	unpack(buf: string): PokemonSet[] | null {
		...
		for (let count = 0; count < 24; count++) {
			...
			// happiness and misc data
			j = buf.indexOf('|', i);
			let misc;
			if (j < 0) {
				if (i < buf.length) {
					const finalSection = buf.substring(i);
					if (finalSection.includes(',')) {
						const parts = finalSection.split(',');
						const isPPUps = parts.every(part => {
							const num = Number(part);
							return !isNaN(num) && num >= 0 && num <= 3;
						});
						
						if (isPPUps && parts.length <= 4) {
							// This is PP Ups data
							set.ppUps = parts.map(x => Number(x));
						} else {
							// This is misc data
							misc = parts.slice(0, 6);
						}
					}
				}
			} else {
			...
			// Process misc data if it exists
			if (misc) {
			...
			// Handle PP Ups in separate section (if j >= 0)
			if (j >= 0) {
				i = j + 1;
				const nextJ = buf.indexOf(']', i);
				
				if (nextJ < 0) {
					if (i < buf.length) {
						const ppUpsString = buf.substring(i);
						set.ppUps = ppUpsString.split(',').slice(0, 4).map(x => 
							Math.max(0, Math.min(3, Number(x) || 0))
						);
					}
					break;
				} else {
					// More Pokémon after this
					if (i !== nextJ) {
						const ppUpsString = buf.substring(i, nextJ);
						set.ppUps = ppUpsString.split(',').slice(0, 4).map(x => 
							Math.max(0, Math.min(3, Number(x) || 0))
						);
					}
	...
	exportSet(set: PokemonSet, { hideStats }: { hideStats?: boolean } = {}) {
		...
		//PPups
		if (set.ppUps && set.ppUps.length > 0) {
			const ppUpStrings = set.ppUps.map((ppUp, index) => {
				if (ppUp > 0 && set.moves[index]) {
					return `${set.moves[index]}: ${ppUp} PP Up${ppUp > 1 ? 's' : ''}`;
				}
				return null;
			}).filter(Boolean);
			
			if (ppUpStrings.length > 0) {
				out += `PP Ups: ${ppUpStrings.join(', ')}  \n`;
			}
		}
	...
	parseExportedTeamLine(line: string, isFirstLine: boolean, set: PokemonSet, aggressive?: boolean) {
		if (isFirstLine) {
			...
			set.moves.push(line);
		} else if (line.startsWith('PP Ups: ')) {
			line = line.slice(8);
			if (!set.ppUps) set.ppUps = [];
			// Parse "Psychic: 1 PP Up, Explosion: 3 PP Ups" format
			const ppUpEntries = line.split(',');
			for (const entry of ppUpEntries) {
				const match = entry.trim().match(/^(.+?):\s*(\d+)\s*PP\s*Ups?$/i);
				if (match) {
					const moveName = match[1].trim();
					const ppUpCount = parseInt(match[2]);
					const moveIndex = set.moves.findIndex(move => 
						move.toLowerCase() === moveName.toLowerCase()
					);
					if (moveIndex >= 0) {
						while (set.ppUps.length <= moveIndex) set.ppUps.push(0);
						set.ppUps[moveIndex] = Math.max(0, Math.min(3, ppUpCount));
					}
				}
			}
		}
		...
		if (buffer.startsWith('[')) {
			try {
				...
				for (const set of team) {
					...
					if (!Array.isArray(set.ppUps)) {
						set.ppUps = [];
					} else {
						// Validate PP Ups (0-3 range, max 4 moves)
						set.ppUps = set.ppUps.slice(0, 4).map((ppUp: number) => 
							Math.max(0, Math.min(3, ppUp || 0))
						);
					}
				}
				return team;
